<template lang="html">
  <div class="kanban-adder" :class="size">
    <div class="add-nested-kanban-button" :class="parent.model" v-if="!isAddingNestedKanban" @click="toggleAddingNestedKanban">
      <i class="fa fa-fw fa-plus" aria-hidden="true"/>
      <span v-if="size=='small'">Add</span>
    </div>

    <template v-if="isAddingNestedKanban">
      <div class="add-nested-kanban-chooser" :class="parent.model" v-if="!kanbanAddableModel">
        <template v-if="parent.model=='folder'">
          <span class="add-folder-chooser folder" @click="selectKanbanAddableModel('folder')">
            <i class="fa fa-fw fa-plus" aria-hidden="true"/>
            Folder
          </span>

          <span class="add-list-chooser middle list" @click="selectKanbanAddableModel('checklist')">
            <i class="fa fa-fw fa-plus" aria-hidden="true"/>
            List
          </span>
        </template>
        ...
        <template v-if="parent.model=='checklist'&&!parent.parent_id">
          <span class="add-folder-chooser section" @click="selectKanbanAddableModel('section')">
            <i class="fa fa-fw fa-plus" aria-hidden="true"/>
            Section
          </span>

          <span class="add-list-chooser middle item" @click="selectKanbanAddableModel('checklist-item')">
            <i class="fa fa-fw fa-plus" aria-hidden="true"/>
            Item
          </span>
        </template>
        ...
        <template v-if="parent.model=='checklist-item'||parent.model=='checklist'&&!!parent.parent_id">
          <span class="add-list-chooser sub-item" @click="selectKanbanAddableModel('checklist-item')">
            <i class="fa fa-fw fa-plus" aria-hidden="true"/>
            Item
          </span>
        </template>

        <span class="cancel-add-nested-kanban cancel" @click="toggleAddingNestedKanban">
          <i class="fa fa-fw fa-times" aria-hidden="true"/>
          Cancel
        </span>
      </div>

      <form class="add-nested-kanban-form" :class="kanbanAddableModel" v-if="kanbanAddableModel" @submit.prevent="addNestedKanban">
        <i class="fa fa-fw add-nested-kanban-input-icon" :class="addNestedKanbanInputIcon" aria-hidden="true"/>
        <input type="text" name="" v-model="nameTitleOrContent" :placeholder="addNestedKanbanInputPlaceholder" v-focus @blur="cancelIfEmpty">
        <i class="fa fa-fw fa-times cancel-add-nested-kanban-icon" aria-hidden="true" @click="toggleAddingNestedKanban"/>
      </form>
    </template>
  </div>
</template>
